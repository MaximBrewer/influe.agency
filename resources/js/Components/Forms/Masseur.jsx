...
export default (props) => {
    ...
    const { masseur = null, genders, localities } = props

    const { zeroPad } = useLayout();

    const [month, setMonth] = useState(masseur && masseur.birthdate ? (new Date(masseur.birthdate)).getMonth() : 0)
    const [year, setYear] = useState(masseur && masseur.birthdate ? (new Date(masseur.birthdate)).getFullYear() : 1980)
    const [day, setDay] = useState(masseur && masseur.birthdate ? (new Date(masseur.birthdate)).getDate() : 1)

    const { data, setData, post, patch, processing, errors, reset, transform } = useForm({
        id: masseur ? masseur.id : null,
        name: masseur && masseur.name ? masseur.name : ``,
        lastname: masseur && masseur.lastname ? masseur.lastname : ``,
        surname: masseur && masseur.surname ? masseur.surname : ``,
        birthdate: masseur && masseur.birthdate ? masseur.birthdate : '01.01.1980',
        tin: masseur && masseur.tin ? `${masseur.tin}` : ``,
        email: masseur && masseur.email ? masseur.email : ``,
        addon: masseur && masseur.addon ? masseur.addon : ``,
        gender: masseur && masseur.gender ? masseur.gender : `male`,
        locality_id: masseur && masseur.locality_id ? masseur.locality_id : localities.data[0].id,
        branch_id: masseur && masseur.branch_id ? masseur.branch_id : ``,
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        if (masseur && masseur.id)
            patch(route('admin.masseurs.update', {
                masseur: masseur.id
            }), {
                onSuccess: () => {
            ...
            post(route('admin.masseurs.store'), {
                onSuccess: () => {
    ...
    return <form onSubmit={submit}>
        ...
        <div className={`flex space-x-8 items-center justify-end`}>
            <Link href={route('admin.masseurs.index')}>
